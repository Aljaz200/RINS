...
void cloud_cb(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    int avg_r = count > 0 ? sum_r / count : 0;
    int avg_g = count > 0 ? sum_g / count : 0;
    int avg_b = count > 0 ? sum_b / count : 0;

    auto isAverageColorGray = [](int r, int g, int b) {
        int threshold = 30;  // Allow a small variation to still be considered gray
        //printf("r: %d, g: %d, b: %d\n", r, g, b);
        bool gray = std::abs(r - g) <= threshold && std::abs(g - b) <= threshold && std::abs(r - b) <= threshold;
        if (gray) {
    ...
    grayDetected = isAverageColorGray(avg_r, avg_g, avg_b);

    if (grayDetected) {
        //printf("Gray cylinder detected, skipping\n");
        if (verbose) {
            std::cerr << "Gray cylinder detected, skipping" << std::endl;
        }
        return;
    }
    else {
