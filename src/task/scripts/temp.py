...
class RingDetector(Node):
    ...
    def setup_voice(self):
        ...
        self.voice_colors = ['red', 'green', 'blue', 'black']
    ...
    def __init__(self):
        ...
        self.ring_pos = self.create_publisher(Marker, "/circle_marker", qos_profile)
        ...
        self.ring_position_threshold = 1.5  # meters, threshold for considering a ring as the same
        ...
        self.marker_timer = self.create_timer(1.0, self.publish_ring_markers)
    ...
    def update_ring(self, position, radius_px, color_name, color_rgb):
        ...
        if robot_position is not None:
            ...
            if distance_to_ring > self.max_detection_distance:
    ...
    def publish_ring_markers(self):
        ...
        for ring_hash in self.ring_positions.keys():
            ...
            self.ring_pos.publish(ring_marker)
            ...
            text_marker.pose.position.x = position[0] + 0.2
    ...
    def image_callback(self, data):
        ...
        for c in candidates:
            ...
            white_background = np.full_like(hsv_image, (255, 255, 255))

            # Inverzna maska - deli, kjer NI ring_mask
            inv_mask = cv2.bitwise_not(ring_mask)

            # Najprej prilepi belo v ozadje, kjer ni ringa
            base = cv2.bitwise_and(white_background, white_background, mask=inv_mask)

            # Nato prilepi obroč, kjer je ring_mask
            ring = cv2.bitwise_and(hsv_image, hsv_image, mask=ring_mask)

            # Združi oboje
            ring_hsv = cv2.add(base, ring)
            ...
            print(color_counts)
            if total_pixels > 50:  # Reduced threshold since we're looking at a smaller area
                    ...
                    if self.pointcloud_data is not None:
                        ...
                        if position_3d is not None:
                            ...
                            if map_position is not None:
                                ...
                                if ring_hash is None: # Too far away
                                    print("TOOOO FAR ----")
                                    continue
                                ...
                                #DODANO OVDE
                                if ring_hash not in self.ring_permanently_published:
                                    self.ring_permanently_published[ring_hash] = False

                                confidence = self.ring_confidence[ring_hash]
                                published = self.ring_published[ring_hash]
                                is_permanent = ring_hash in self.ring_permanently_published and self.ring_permanently_published[ring_hash]

                                if is_permanent:
                                    status = f"PERMANENTLY PUBLISHED with hash {ring_hash}"
                                elif published:
                                    status = f"PUBLISHED with hash {ring_hash}"
                                else:
                                    status = f"CONFIDENCE {confidence}/{self.min_confidence_threshold} with hash {ring_hash}"
